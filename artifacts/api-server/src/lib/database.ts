...
const noOpQueries = {
  ...
  saveAudioOnly: { run: noop },
  updateReceiptResult: { run: noop },
  saveReceiptOnly: { run: noop },
...
function setupSqlite() {
  ...
  return {
    ...
    saveAudioOnly: db.prepare(`UPDATE expense_runs SET audio_result = ? WHERE id = ?`),
    updateReceiptResult: db.prepare(`UPDATE expense_runs SET status = 'step3', receipt_result = ? WHERE id = ?`),
    saveReceiptOnly: db.prepare(`UPDATE expense_runs SET receipt_result = ? WHERE id = ?`),
